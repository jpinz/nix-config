{
            ...
            }
            {
              ...
              widgets = [
                ...
                # SABnzbd Status Widget
                {
                  ...
                  title = "SABnzbd";
                  cache = "30s";
                  url = "http://localhost:8080/sabnzbd/api?output=json&apikey=\${SABNZBD_API_KEY}&mode=queue";
            ...
            }
            {
              ...
              widgets = [
                # Sonarr Upcoming Widget
                {
                  ...
                  cache = "15m";
                  options = {
                    base-url = "http://localhost:8989/sonarr";
                          ...
                          {{ range $episodes }}
                            <li>
                              ...
                              <div class="flex justify-between">
                                <span class="color-subdue">S{{ .Int "seasonNumber" }}E{{ .Int "episodeNumber" }}</span>
                                <span class="color-primary">{{ .String "airDateUtc" | parseTime "2006-01-02T15:04:05Z" | formatTime "Jan 2" }}</span>
                              </div>
                            </li>
                          {{ end }}
                        </ul>
                ...
                # Radarr Upcoming Widget
                {
                  ...
                  cache = "15m";
                  options = {
                    base-url = "http://localhost:7878/radarr";
  ...
  # Glance needs to bind to port 80, which requires elevated privileges
  systemd.services.glance.serviceConfig = {
    AmbientCapabilities = [ "CAP_NET_BIND_SERVICE" ];
    CapabilityBoundingSet = [ "CAP_NET_BIND_SERVICE" ];
  };

  # Open port 80 for the dashboard
